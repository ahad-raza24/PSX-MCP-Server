...
from psx_mcp.client import PSXClient  # noqa: E402
...
async def demo_new_tools():
    ...
    try:
        ...
        filtered_data = [
            point
            for point in eod_data
            if start_date.timestamp() <= point["timestamp"] <= end_date.timestamp()
        ]

        print(f"  Found {len(filtered_data)} data points in last 30 days")
        if filtered_data:
            latest_time = datetime.fromtimestamp(filtered_data[0]['timestamp'])
            latest_price = filtered_data[0]['price']
            print(f"  Latest: {latest_time.strftime('%Y-%m-%d')} - "
                  f"Price: {latest_price}")

            oldest_time = datetime.fromtimestamp(filtered_data[-1]['timestamp'])
            oldest_price = filtered_data[-1]['price']
            print(f"  Oldest: {oldest_time.strftime('%Y-%m-%d')} - "
                  f"Price: {oldest_price}")
        ...
        if intraday_data:
            ...
            recent_data = [
                point for point in intraday_data if point["timestamp"] >= two_hours_ago
            ]

            print(f"  Found {len(recent_data)} data points in last 2 hours")
            if recent_data:
                latest_time = datetime.fromtimestamp(recent_data[0]['timestamp'])
                latest_price = recent_data[0]['price']
                print(f"  Latest: {latest_time.strftime('%H:%M:%S')} - "
                      f"Price: {latest_price}")

                min_vol = min(p['volume'] for p in recent_data)
                max_vol = max(p['volume'] for p in recent_data)
        ...
        for symbol in symbols_to_check:
            ...
            if stock:
                print(
                    f"  {symbol}: O:{stock['open_price']} "
                    f"H:{stock['high_price']} L:{stock['low_price']} "
                    f"C:{stock['current_price']} V:{stock['volume']}"
                )
        ...
        if intraday_data:
            ...
            closest_point = min(
                intraday_data, key=lambda x: abs(x["timestamp"] - one_hour_ago)
            )
            print(f"  Closest price to 1 hour ago: {closest_point['price']}")
            time_str = datetime.fromtimestamp(
                closest_point['timestamp']
            ).strftime('%H:%M:%S')
...
def show_all_tools():
    ...
    advanced_tools = [
        "get_eod_data_by_date_range(symbol, start_date, end_date) - EOD data",
        "get_intraday_data_by_time_range(symbol, start_time, end_time) - Intraday",
        "get_ohlcv_data(symbol) - OHLCV data for specific stock",
        "get_multiple_stocks_ohlcv(symbols) - OHLCV for multiple stocks",
        "get_stock_price_at_time(symbol, timestamp) - Price at timestamp",
        "get_volume_analysis(symbol, days) - Volume analysis",
    ]
